...
6. Records actions to memory for learning
...
class Orchestrator:
    ...
    async def approve(self, task_id: str, approved_indices: list[int] = None) -> Task:
        ...
        try:
            ...
            # Record to memory for pattern learning
            memory.record_action(
                skill=task.skill.name,
                action_type="execute",
                target=task.request[:100],  # First 100 chars of request
                outcome="success",
                metadata={
                    "task_id": task.id,
                    "actions_approved": len(approved_indices),
                    "actions_total": len(task.plan.actions) if task.plan else 0,
                }
            )
    ...
    async def reject(self, task_id: str, reason: str = None) -> Task:
        ...
        # Record rejection to memory - helps learn what user doesn't want
        if task.skill:
            memory.record_action(
                skill=task.skill.name,
                action_type="reject",
                target=task.request[:100],
                outcome="cancelled",
                metadata={"reason": reason or "User rejected"}
            )
